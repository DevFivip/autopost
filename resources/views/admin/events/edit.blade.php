<x-tomato-admin-container label="{{trans('tomato-admin::global.crud.edit')}} {{__('Event')}} #{{$model->id}}">
    <x-splade-form class="flex flex-col space-y-4" action="{{route('admin.events.update', $model->id)}}" method="post" :default="$model">
          ...
          <x-splade-select :label="__('Customer')" :placeholder="__('Customer')" name="customer_id" remote-url="/admin/customers/api" remote-root="data" option-label=fullname option-value="id" choices/>
          ...
          <x-splade-input :label="__('Status')" :placeholder="__('Status')" type='number' name="status" />
